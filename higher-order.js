...
//Code Here
let totalPopulation = populations.reduce(function(total, element, index, array){ return total + element
})
...
let orderTotals = orders.map(function(element, index, array){
  return orders[index].price + (orders[index].tax * orders[index].price)
})
...
.reduce(function(total, element, index, array){
    return  total + array[index].price
  },0)
